...
var _ = Describe("For primary IPs", Ordered, Label("primaryips"), func() {
	...
	When("getting prices", func() {
		...
		It("should get zero for incorrect values", func() {
			...
			Expect(testutil.ToFloat64(sut.GetHourly().WithLabelValues("test-primaryipv4", "nbg1", "ipv4", "e2e_suite_test"))).Should(BeNumerically("==", 0))
			Expect(testutil.ToFloat64(sut.GetHourly().WithLabelValues("test-primaryipv4", "fsn1", "ipv6", "e2e_suite_test"))).Should(BeNumerically("==", 0))
			Expect(testutil.ToFloat64(sut.GetHourly().WithLabelValues("test-primaryipv4", "fsn1", "ipv4", "e3e_suite_test"))).Should(BeNumerically("==", 0))

			By("Checking IPv6 prices")
			Expect(testutil.ToFloat64(sut.GetHourly().WithLabelValues("invalid-name", "fsn1", "ipv6", "e2e_suite_test"))).Should(BeNumerically("==", 0))
			Expect(testutil.ToFloat64(sut.GetHourly().WithLabelValues("test-primaryipv6", "nbg1", "ipv6", "e2e_suite_test"))).Should(BeNumerically("==", 0))
			Expect(testutil.ToFloat64(sut.GetHourly().WithLabelValues("test-primaryipv6", "fsn1", "ipv4", "e2e_suite_test"))).Should(BeNumerically("==", 0))
			Expect(testutil.ToFloat64(sut.GetHourly().WithLabelValues("test-primaryipv6", "fsn1", "ipv6", "e3e_suite_test"))).Should(BeNumerically("==", 0))
		})
	})
})
